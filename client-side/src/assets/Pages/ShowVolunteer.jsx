...
const ShowVolunteer = () => {
  ...
  const [regs, setRegs] = useState([]); // approved registrations of events managed by current user
  ...
  const [kickTarget, setKickTarget] = useState(null); // registration to kick
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Backend should return approved registrations for events managed by current EVENT_MANAGER
  ...
  const refresh = async () => {
    try {
      ...
      console.warn('refresh volunteers failed', e);
    }
  };

  // Build campaign list from approved registrations
  ...
  const openKickConfirm = (reg) => { setKickTarget(reg); setConfirmKickOpen(true); };
  const closeKickConfirm = () => { setKickTarget(null); setConfirmKickOpen(false); };

  const kickVolunteer = async () => {
    if (!kickTarget) return;
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:4000/registrations/${kickTarget.id}/status`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
